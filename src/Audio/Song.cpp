...
Song::Song(ghc::filesystem::path path, bool repeat)
    : repeat(repeat), valid(false) {
  if (!Load(path, repeat)) {
    Logger::error(AUDIO) << "Song::Song() argument 'path' resulted in an error!"
                         << std::endl;
  }
}
...
bool Song::Load(ghc::filesystem::path path, bool repeat) {
  ...
  if (!fp) {
    Logger::error(AUDIO) << "Unable to open song '" << path << "'!"
                         << std::endl;
    return false;
  }
  fseek(fp, 0, SEEK_END);
  ...
  alBufferData(buffer, AL_FORMAT_MONO16, static_cast<const ALvoid *>(ptr),
               static_cast<ALsizei>(size), SAMPLING_RATE);
  Audio::PrintError();
